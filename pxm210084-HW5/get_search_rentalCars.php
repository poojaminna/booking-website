...
if ($_SERVER["REQUEST_METHOD"] === "GET") {
    ...
    $sql = "SELECT * FROM rentalCars WHERE city = '$city' AND checkInDate = '$checkInDate' AND checkOutDate = '$checkOutDate' ";
    // echo "origin" .$origin;
    ...
    if($result->num_rows == 0){
        $sql = "SELECT * FROM rentalCars WHERE city = '$city' ";
        $result = $conn->query($sql);
        if (!$result) {
            echo "Error: hiii" . $sql . "<br>" . $conn->error;
            exit; // Exit the script to prevent further execution
